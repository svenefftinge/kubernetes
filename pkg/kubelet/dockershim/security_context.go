...
func modifyContainerConfig(sc *runtimeapi.LinuxContainerSecurityContext, config *dockercontainer.Config) error {
	if sc == nil {
		return fmt.Errorf("LinuxContainerSecurityContext is nil")
	}
	if sc.RunAsUser != nil {
	...
	if sc.RunAsGroup != nil {
		if user == "" {
			return fmt.Errorf("Group is specified without a User, %s", sc.RunAsGroup)
		}
		gid := strconv.FormatInt(sc.GetRunAsGroup().Value, 10)
		user = fmt.Sprintf("%s:%s", config.User, gid)
	}
